...

{
    ...
    return [self initWithUploadId:uploadId
                    ...
                    finalMetadata:uploadMetadata
                            state:TUSResumableUploadStateCheckingFile ///XnBay///TUSResumableUploadStateUploadingFile
                        uploadUrl:uploadUrl];
...
- (BOOL)createFile
{
    ...
    self.currentTask = [self.delegate.session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error){
        ...
        if (error != nil || httpResponse == nil){
            switch(error.code){
                ...
                default:
                    ...
                    if (self.rertyCount == -1){
                        ...
                        if (block){
                            [[NSOperationQueue currentQueue] addOperationWithBlock:^{
                                block([[NSError alloc] initWithDomain:TUSErrorDomain code:NSURLErrorCannotConnectToHost userInfo:@{@"responseCode": @("No any response")}]);
                            }];
                        }
                    }
            ...
            TUSLog(@"Server error, stopping with %ld @createFile", (long)httpResponse.statusCode);
            ...
            if (self.rertyCount == -1){
                ...
                if (block){
                    [[NSOperationQueue currentQueue] addOperationWithBlock:^{
                        block([[NSError alloc] initWithDomain:TUSErrorDomain code:NSURLErrorCannotConnectToHost userInfo:@{@"responseCode": @(httpResponse.statusCode)}]);
                    }];
                }
            }
        ...
        if (error != nil || httpResponse == nil){
            switch(error.code){
                ...
                default:
                    ...
                    //TODO: Fail after a certain number of delayed attempts
                    delayTime = DELAY_TIME;
                    TUSLog(@"Error or no response during attempt to check file, retrying %i", self.attempts);
                    break;
            ...
            TUSLog(@"Server error, stopping with %ld", (long)httpResponse.statusCode);
            [weakself stop]; // Will prevent continueUpload from doing anything
            // Make the callback after the current operation so that the rest of the method will finish.
            // Store the block so that we know it will be non-nil in the closure.
            TUSUploadFailureBlock block = weakself.failureBlock;
...
-(BOOL)uploadFile
{
    ...
    self.currentTask = [self.delegate.session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error){
        ...
        if (error != nil || httpResponse == nil){
            ...
            TUSLog(@"Server error, stopping with %ld @uploadFile", (long)httpResponse.statusCode);
            weakself.state = TUSResumableUploadStateCheckingFile;
            [weakself stop]; // Will prevent continueUpload from doing anything
            // Make the callback after the current operation so that the rest of the method will finish.
            // Store the block so that we know it will be non-nil in the closure.
